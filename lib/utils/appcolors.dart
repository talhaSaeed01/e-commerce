import 'package:flutter/material.dart';

class Appcolors {
  static var red = Colors.red;
  static var white = Colors.white;
  static var boxcoloronboarding = const Color(0xFF464447);
  static var caustomOCimage = const Color(0xFFE7E8E9);
  static var black = Colors.black;
  static var browncolorbutton = const Color(0xFF2D201C);
  static var grey = const Color(0xFF9B9B9B);
  static var circlecontainercolor = const Color(0x7FC2C6B5);
  static var discover1TileColor = const Color(0xFFA3A798);
  static var discover2TileColor = const Color(0xFF898280);
  static var discover3TileColor = const Color(0xFF44565C);
  static var discover4TileColor = const Color(0xFFB9AEB2);
  static var purpleColor = const Color.fromARGB(255, 160, 93, 124);
  static var heartcolor = Colors.red;
  static var starcolorblue = const Color.fromARGB(255, 18, 94, 156);
  static var blue = Colors.blue;
  static var green = const Color.fromARGB(255, 55, 145, 101);
  static var yellow = Colors.yellow;
}
